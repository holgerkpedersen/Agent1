...
All metric/filter/threshold types are ``TypedDict``s — plain ``dict``
objects at runtime.  They are validated via subscription and discriminated
by key presence; ``isinstance()`` against a TypedDict raises ``TypeError``.

Module-level convenience wrappers (:func:`validate_command_metric`,
:func:`validate_task_metric`, :func:`validate_performance_record`,
:func:`validate_filter`, :func:`verify_threshold`) take the owning validator
as first argument and return a boolean instead of raising, e.g.::

    ok = validate_task_metric(MetricSchemaValidator(), metric)
"""
...
class MetricSchemaValidator:
    ...
    def validate_command_metric(self, metric: CommandMetric) -> bool:
        if not _within_bounds(float(metric["execution_time_ms"]), NON_NEGATIVE_BOUND, math.inf):
            raise ValueError("execution_time_ms must be non-negative")
        if not _within_bounds(float(metric["memory_usage_mb"]), NON_NEGATIVE_BOUND, math.inf):
            raise ValueError("memory_usage_mb must be non-negative")
        if not _within_bounds(
            float(metric["cpu_utilization_percent"]), MIN_CPU_UTILIZATION, MAX_CPU_UTILIZATION
        ):
        ...
        rc = int(metric["return_code"])
        if rc < EXIT_CODE_MINIMUM or rc > EXIT_CODE_MAXIMUM:
            raise ValueError(
    ...
    def validate_task_metric(self, metric: TaskMetric) -> bool:
        if not _within_bounds(float(metric["duration_seconds"]), NON_NEGATIVE_BOUND, math.inf):
            raise ValueError("duration_seconds must be non-negative")
        if not _within_bounds(
            float(metric["success_rate"]), SUCCESS_RATE_MINIMUM, SUCCESS_RATE_MAXIMUM
        ):
            raise ValueError("success_rate must fall within [0.0, 1.0]")
        if not _within_bounds(
            float(metric["resource_consumption_units"]), NON_NEGATIVE_BOUND, math.inf
        ):
    ...
    def validate_performance_record(self, record: PerformanceRecord) -> bool:
        if not _within_bounds(float(record["timestamp"]), NON_NEGATIVE_BOUND, math.inf):
            raise ValueError("timestamp must be non-negative")
        inner = record["record_type"]
        # TypedDicts cannot be isinstance-checked: discriminate by keys.
        if "execution_time_ms" in inner:
            self.validate_command_metric(inner)  # type: ignore[arg-type]
        elif "duration_seconds" in inner:
            self.validate_task_metric(inner)  # type: ignore[arg-type]
        else:
            raise TypeError(
                "record_type must carry CommandMetric or TaskMetric fields"
            )
        return True
...
class FilterParameterValidator:
    ...
    def validate_filter(self, filter_spec: DashboardFilter) -> bool:
        if not _within_bounds(float(filter_spec["start_time"]), NON_NEGATIVE_BOUND, math.inf):
            raise ValueError("start_time must be non-negative")
        if not _within_bounds(float(filter_spec["end_time"]), NON_NEGATIVE_BOUND, math.inf):
        ...
        if not _within_bounds(
            float(filter_spec["minimum_cpu_utilization"]),
            MIN_CPU_UTILIZATION, MAX_CPU_UTILIZATION,
        ):
            raise ValueError("minimum_cpu_utilization must fall within [0.0, 100.0]")
        if not _within_bounds(
            float(filter_spec["maximum_memory_usage_mb"]), NON_NEGATIVE_BOUND, math.inf
        ):
...
class ThresholdConsistencyVerifier:
    ...
    def verify_threshold(self, threshold: AlertThreshold) -> bool:
        ...
        warning_limit = float(threshold["warning_limit"])
        error_limit = float(threshold["error_limit"])
...
# -- boolean-returning convenience wrappers ----------------------------------


def validate_command_metric(
    validator: MetricSchemaValidator, metric: CommandMetric
) -> bool:
    """True when *metric* passes *validator*'s schema checks."""
    try:
        return bool(validator.validate_command_metric(metric))
    except (ValueError, TypeError, KeyError):
        return False


def validate_task_metric(
    validator: MetricSchemaValidator, metric: TaskMetric
) -> bool:
    """True when *metric* passes *validator*'s schema checks."""
    try:
        return bool(validator.validate_task_metric(metric))
    except (ValueError, TypeError, KeyError):
        return False


def validate_performance_record(
    validator: MetricSchemaValidator, record: PerformanceRecord
) -> bool:
    """True when *record* passes *validator*'s schema checks."""
    try:
        return bool(validator.validate_performance_record(record))
    except (ValueError, TypeError, KeyError):
        return False


def validate_filter(
    validator: FilterParameterValidator, filter_spec: DashboardFilter
) -> bool:
    """True when *filter_spec* passes *validator*'s bound checks."""
    try:
        return bool(validator.validate_filter(filter_spec))
    except (ValueError, TypeError, KeyError):
        return False


def verify_threshold(
    verifier: ThresholdConsistencyVerifier, threshold: AlertThreshold
) -> bool:
    """True when *threshold* is internally consistent per *verifier*."""
    try:
        return bool(verifier.verify_threshold(threshold))
    except (ValueError, TypeError, KeyError):
        return False
