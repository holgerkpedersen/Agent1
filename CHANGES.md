## 2026-09-23 - fix: regenerated regression pins no longer import compile_trace from harnessfix.reader

**Change**: `harnessfix/review.py` (the `export_regression_test()` template string now emits `from harnessfix.htir import compile_trace` instead of `from harnessfix.reader import compile_trace` — `compile_trace` has lived in `harnessfix.htir` since the HTIR refactor; `harnessfix.reader` only defines `read_trace`/`task_id_of`). `tests/test_harnessfix_review.py` (+1: `test_export_regression_test_pin_imports_compile_trace_from_htir` — exports a pin from a real failed trace, asserts the generated source imports `compile_trace` from `harnessfix.htir` and never mentions `harnessfix.reader`, then `exec_module`s the generated file so a stale import raises `ImportError` exactly as pytest collection would). `docs/issues/PRE-EXISTING-TEST-FAILURES.md` (status updated: all three tracked failures are resolved on master). `CHANGES.md` (this entry).

**Reason**: The pin template emitted an import from a module that no longer defines `compile_trace`, so every `export_regression_test` run produced a pin that dies at collection with `ImportError: cannot import name 'compile_trace' from 'harnessfix.reader'` — the exact failure that broke full-suite collection in the tracked issue (docs/issues/PRE-EXISTING-TEST-FAILURES.md #1). The generated artifact on this machine had already been regenerated with the correct import, but the generator was still broken: regenerating pins from `review.json` would have re-produced the broken file. RED was watched first: the new test failed with `assert 'from harnessfix.htir import compile_trace' in text` before the one-line template change. Verified: `tests/test_harnessfix_review.py` 12 passed; the standalone generated pin `reports/harnessfix/generated/test_review_7b31b6434d1840bc8e809f4e43335d16.py` passes (1 passed); full suite 2575 passed, 7 skipped in 0:16:04.

**Files**: harnessfix/review.py, tests/test_harnessfix_review.py (+1 test, 12 passed), docs/issues/PRE-EXISTING-TEST-FAILURES.md, CHANGES.md.

## 2026-09-22 - fix: emoji detector no longer flags typography (Sk category dropped from the fallback)

**Change**: `agent_core/text_policy.py` (`is_emoji_char` falls back to `unicodedata.category(ch) == "So"` instead of `in ("So", "Sk")`; module docstring updated to match). `tests/test_text_policy.py` (+3: seven parametrized accent/macron/tone-bar code points asserted not-emoji, the five Fitzpatrick skin-tone modifiers U+1F3FB-U+1F3FF asserted still-emoji, and U+231A WATCH asserted still-caught through the So path). `.decisions.json` (#079 decision text amended: `So/Sk categories` -> `So category`, with the amendment and its reasoning recorded inline). `CHANGES.md` (this entry).

**Reason**: The blanket `category == "Sk"` fallback contradicted the module's own contract ("Typography (em dash, <=, accents, CJK) is never flagged") and decision #079's own rationale ("accents ... never flagged"). Measured before the fix: of the 123 Sk code points in Unicode, 118 lie outside every `_EMOJI_RANGES` span and all 118 were reported as emoji — U+00B8 CEDILLA, U+00A8 DIAERESIS, U+00AF MACRON, U+00B4 ACUTE ACCENT, the tone bars, the Greek accent marks. The only five genuinely-emoji Sk code points (the Fitzpatrick skin-tone modifiers) already sit inside `_EMOJI_RANGES`, so the Sk branch contributed ZERO additional emoji detection: it was pure false positives. Consequence observed in the real repo: `scan_tree` flagged the docstring in `tests/test_subprocess_utf8_encoding.py` that illustrates cp1252 mojibake (`"\u00b8".encode("cp1252")`), so audit check 6 failed on a file containing no emoji at all. Verified after the fix: Sk-outside-range findings drop 118 -> 0, the five Fitzpatrick modifiers and the So-category emoji (U+231A WATCH, U+00A9, U+00AE, U+2122, U+23E9, U+25AA, U+3297) are still detected, `scan_tree` over the real repo returns no findings, `scripts/audit_invariants.py` reports OK, and `tests/test_text_policy.py` is 35 passed. RED was watched first: the 7 new accent cases failed with `AssertionError: U+00B8 is typography, not emoji` before the one-line implementation change.

**Files**: agent_core/text_policy.py, tests/test_text_policy.py (+3 tests, 35 passed), .decisions.json (#079 amended), CHANGES.md.

## 2026-09-18 - feat: persist LLM outcomes as experiences; stop tests writing live runtime state

**Change**: `Agent._record_llm_experience(action, outcome, context=None, success=None) -> int | None` (agent.py) appends one row to the `experiences` table of `agent_memory.db` (path derived from `AGENT_MEMORY_JSON_PATH`). It creates the table when absent using DDL byte-compatible with the schema the Memory MCP server reads (`timestamp, action, outcome, context, success`, same column order), serialises `context` as sorted-key JSON, derives `success` from `outcome >= 0.5` when not given, rejects an out-of-range `outcome` without writing, and — per its contract — NEVER raises (any failure, including a missing/unwritable DB, is a silent no-op). `_run_chained_tool_loop` calls it once per completed run with `action="llm_decision"`, `outcome = 1.0 if not llm_error else 0.0`, `success = not llm_error and verdict in ("answer", "cap")` and `context = {model, profile, verdict, continuations, final_text_len}` — after the inner tool loop, so the recorded verdict describes the finished run and a transient-failure retry cannot double-write. `conftest.py::_isolate_from_real_tree_and_beacons` (already autouse) additionally redirects `CHAT_HISTORY_JSON_PATH` and `AGENT_MEMORY_JSON_PATH` — and therefore the derived DB — into the per-test temp dir. New `tests/test_experience_recording.py` (18 tests): direct-helper contract (rowid, MCP-compatible schema, pre-existing table reused, out-of-range/non-numeric rejected, `success` threshold boundaries, sorted JSON context, never-raises on an unwritable location), end-to-end `chat_nlp` recording plus the isolation pins.

**Reason**: The self-improvement loop needs outcome telemetry it can reconcile with ground truth, and `ToolLoopRunner.termination_reason` is the surface that already distinguishes a real answer (`answer`, `cap`) from a stuck run (`stuck`, `no_progress`). Writing it through the same `experiences` table the Memory MCP server reads keeps one reconcilable source rather than a second, private store, and doing it in the harness layer (no model or prompt change) respects the repair-scope restriction. Separately, `chat_nlp` tests that sandboxed only `CHAT_HISTORY_JSON_PATH` still rewrote the LIVE `agent_memory.json` and appended phantom `llm_decision` rows to the LIVE `agent_memory.db`; those rows are indistinguishable from genuine telemetry afterwards and skew `evolution_summary` for a real user, so the redirect is pinned by tests. Verified: the increment's 18 tests and the neighbouring suites (`test_agent_robustness` 20, `test_tool_loop_nlp` 78, `test_quickwins_2026_08_25` 15, `test_quickwins_batch2` 15) pass, and both a sequential and a concurrent (with-coverage) invocation of the memory-path suites leave the live `agent_memory.db` row count, `agent_memory.json` and `chat_history.json` mtimes untouched.
...
**Reason**: `python -m pytest tests/ -q --no-cov` was killed at the model's 120s default and again at its 300s retry. Root cause: an explicit `tests/` path disqualified the run from BOTH safety nets — conftest's watchdog (`_is_full_run` = "no positional path") and the run tool's `PYTEST_FULL_SUITE_TIMEOUT` injection — even though `tests/` IS the whole suite (`testpaths = ["tests"]`). The 20% headroom was also thin, and an aborted run recorded nothing (`os._exit` skips `pytest_sessionfinish`), so a too-small budget could overrun forever. Now the suite budget is `max(floor, last * 1.5)`, applies to `pytest tests/`, and self-corrects upward on abort; the docs add `--lf` (re-run only failures) and `--testmon` (affected tests only) so iterative work never rebuilds the whole suite.

**Files**: conftest.py, agent.py, .env.example, AGENTS.md, pyproject.toml, .gitignore, CHANGES.md, tests/test_conftest_budget.py, tests/test_pytest_timeout_injection.py.
...
**Reason**: The psutil collection fix let the full suite run on the Linux runner for the first time, exposing four latent failures that only ever ran on Windows locally. (1) The failover test passed locally only because the gitignored `.env` set the chain to `deepseek-v4.1-flash`; on CI there is no `.env`, so `load_agent_settings()` correctly returned the catalog default `deepseek-v4-flash` and the hardcoded expectation mismatched — a test that measures the developer's machine, not the code. (2) `_scan_workspace_context` produced different output per OS because `os.walk` yields entries in filesystem order; on Linux `b.py` won the budget before `a.py`, and the budget counted only content lines while emitting header lines, so `len(combined.splitlines()) <= 6000` was not actually guaranteed. (3) `shell_info()` gates its cmd/PowerShell detection behind `sys.platform == "win32"`, so the two monkeypatched tests asserted Windows results on Linux where the code intentionally returns `/bin/bash`.

**Files**: tests/test_failover_chain.py, agent_core/commands/workflow_cmd.py, tests/unit/test_subprocess_utils.py. Verified: the three affected files pass locally (47) and with the related failover/workflow/budget suites (87); the blocking `ruff --select F821` gate is clean.
...
**Reason**: The model routinely called `run(command="python -m pytest -q --no-cov", timeout=600)`. On this machine the full suite needs more than 600s (the in-suite watchdog budget is 900s, from `PYTEST_FULL_SUITE_TIMEOUT` in `.env`), so the run tool killed the process tree at 600s every time — wasting ~10 minutes and repeating forever because the tool never consulted the one value the watchdog already treats as authoritative. Tool and watchdog budgets can no longer disagree.

**Files**: agent.py, tests/test_pytest_timeout_injection.py (new, 23 tests: full-vs-targeted command detection incl. `set ... &&`, `2>&1`, `>nul`, `--ignore`; env/.env resolution incl. last-run margin and process-env precedence; `_nlp_run` wiring via a Popen stand-in proving an injected 900/1800s wins over a model 600/30s while a targeted run stays capped at 600). Full targeted run green.

## 2026-09-16 - fix: in-loop compaction - char-budget trigger, phantom summary, unbounded summarize payload

**Change**: `agent_core/llm/tool_loop.py` (three fixes in the in-loop compaction path: (1) the trigger was `len(current_messages) > compact_max_messages` only, so `compact_char_budget` was ignored at the call site - the guard now fires on count **or** `_total_chars(...) > compact_char_budget`, matching what `_split_for_compaction` already enforced; (2) the call site defaulted `summary = "dropped to save context"`, a literal that the note rendered as `(working summary: dropped to save context)` - the note now states a working summary only when one was actually produced, via a `{summary_clause}` template slot that is empty for `None`/blank summaries; (3) the summarize prompt embedded `json.dumps(dropped, default=str)` unbounded, re-sending the entire evicted prefix - new `_summarize_payload()` caps it at `_SUMMARIZE_MAX_CHARS` (20_000) with an explicit `...[truncated]` marker).

**Reason**: The summarize strategy exists to keep prompts small, but each of the three defects re-introduced the growth it was meant to prevent. (1) A handful of large tool results could sit under the message-count cap forever while being re-sent in full every iteration - the documented `compact_char_budget` was dead at the call site. (2) Reporting a summary that was never written sends the model looking for memory that does not exist, and made the no-summary and real-summary cases textually indistinguishable (defeating the diagnosis signatures decision #020 protects); a blank summary now renders identically to `None`. (3) `summarize` on a 400KB dropped prefix built a ~400KB summarizer prompt - the same timeout the compaction guards against. Fail-open is preserved: a summarizer exception or an empty response now yields `None` (note without the summary clause) rather than a fabricated one.

**Files**: agent_core/llm/tool_loop.py, tests/test_loop_compaction.py (new `TestCharBudgetTrigger` - char budget triggers compaction with no count breach, asserted both at `_split_for_compaction` level and end-to-end through the real `ToolLoopRunner` via the `GUARD_COMPACT` trace event; `TestSummarizePayloadCap` - capped/untouched payload plus a spy proving the *summarizer* prompt specifically stays bounded for a 400KB dropped prefix; `test_absent_summary_is_not_advertised` + `test_real_summary_is_included` + `test_note_text_is_stable_for_summary_and_no_summary` for the note contract), tests/test_tool_loop_defects.py (compaction-note assertions updated to the new note form).

## 2026-09-14 - fix: two more HTTP 400 shapes in the outbound message array (invalid JSON in truncated args, unanswered tool_calls)

**Change**: `agent.py::_strip_tool_args` (a tool-call argument longer than 500 chars was replaced by `args[:200] + "... [truncated]"` — a mid-value cut of a JSON string; it is now replaced by the valid compact object `{"_truncated": true, "_original_len": N}` so the call name survives, the history fits the char budget, and the payload still parses), `agent_core/llm/tool_loop.py` (after a batch executes, every announced `tool_call_id` of the most recent assistant `tool_calls` message that has no following `tool` message gets a synthetic "not executed — the loop stopped after a repeated call" result appended, so a mid-batch `stuck` break can no longer leave a half-answered batch).

**Reason**: Both are 400s emitted by strict gateways (opencode Console Go), not by us: "Assistant tool call function.arguments must be valid JSON" (the truncated fragment was persisted to `chat_history.json` and re-sent on every later turn, so one big `write` poisoned the whole session) and "An assistant message with 'tool_calls' must be followed by tool messages responding to each 'tool_call_id'" (the sequential loop `break`s on `stuck`, dropping the results of calls already announced in the same batch). Same family as commit 1d04705 (corrupt-emitted arguments) — the outbound array must be structurally valid JSON *and* structurally complete before it leaves the process.

**Files**: agent.py, agent_core/llm/tool_loop.py, tests/test_quickwins_2026_08_25.py (`test_stripped_tool_args_stay_valid_json` — parses every stripped arguments string, asserts the tool name survives), tests/test_tool_loop_nlp.py (`test_stuck_batch_backfills_unanswered_tool_calls` — runs the real `ToolLoopRunner` with a scripted repeat+write batch and asserts every announced id has a following `tool` message). Full suite: 2322 passed / 7 skipped; `scripts/audit_invariants.py` OK.

## 2026-08-30 - feat: first-class `propose` mode — generate + validate in memory, emit a reviewable bundle, never write the tree

**Change**: New `agent_core/commands/proposal_core.py` (in-memory `[PATCH:]`/`[FILE:]` apply via the existing `patch_utils` anchored diff, reuse of `implement_cmd`'s pre-write gates, optional pytest/security validation against a temp copy of the workspace, and bundle emission as `proposal.md` + a `git apply`-compatible `proposal.patch`). New `propose` REPL command (`agent_core/commands/propose_cmd.py`) — a thin front-end that forwards to `implement --propose` so 100% of generation/parsing/gating logic is shared. `implement` gains `--propose`/`--validate`/`--out`; `fix --mypy` gains `--propose` (captures candidate fixes in memory and emits them as a bundle instead of applying). Registered in `agent.py::_register_commands`. Docs: README.md, USAGE.md (new "Propose mode" section). Tests: tests/test_propose.py (12 — in-memory apply, gate reuse, bundle/patch emission, and the NEVER-WRITE-THE-TREE invariant).

**Reason**: The autonomous driver protects the tree with `git stash` checkpoints and reverts via `stash pop` — a rollback guarantee that still leaves a dirty-tree window and can pop the wrong stash. `propose` is strictly stronger: the agent never mutates the working tree at all. The checkpoint becomes the artifact (a diff + rationale + test result) that a human or CI merges with `git apply`, carrying the base `HEAD` + working-tree status so CI can assert the base matches before applying.

**Files**: agent_core/commands/proposal_core.py (new), agent_core/commands/propose_cmd.py (new), agent_core/commands/implement_cmd.py (`--propose`/`--validate`/`--out` flags + propose branch after the import gate), agent_core/commands/fix_cmd.py (`--propose`/`--out` flags + `_capture_proposed` + bundle emit in `_fix_mypy`), agent.py (registration), README.md, USAGE.md, CHANGES.md. Tests: tests/test_propose.py (12 passed).

## 2026-08-26 - feat: MCP (Model Context Protocol) consumer - REPL command, web UI, opt-in LLM bridge

**Change**: New `agent_core/mcp/` package: `jsonrpc.py` (typed JSON-RPC 2.0 framing; malformed server frames raise `JsonRpcError` instead of crashing), `transports.py` (`StdioTransport`: argv-list Popen with reader thread + id-matched responses, never shell=True, child killed on disconnect; `HttpTransport`: JSON and SSE bodies; every request wall-clock clamped 1-600s), `config.py` (validated `mcp.json` at repo root, gitignored, atomic writes; secret material only via `secret:<keyring>` or `${ENV}` references resolved fail-closed at connect), `client.py` (initialize handshake, tools/resources; arguments validated against each tool's declared JSON schema BEFORE the call leaves the process; results capped at 20k chars), `manager.py` (lock-guarded multi-server lifecycle, process-wide singleton shared by REPL/dashboard/NLP). New `mcp` REPL command (`agent_core/commands/mcp_cmd.py`: status/add/remove/connect/disconnect/tools/call/resources/read/expose) - the ONLY writer of mcp.json. Dashboard gains `/mcp` page (static/mcp.html: server table, connect/disconnect, schema-template tool caller, 3s polling, ttheme-styled) plus GET `/api/mcp/state|tools` and POST `/api/mcp/connect|disconnect|call` in dashboard_api.py. NLP bridge: two new tools `mcp_tools`/`mcp_call` in tool_schemas.py + agent.py handlers, reachable only for servers with `expose_to_llm: true`.

**Reason**: The user asked for a flexible-but-safe interface to consume MCP services. Trust is layered by construction: config changes require the human REPL; the web page can invoke tools on already-connected servers but has no endpoint that reads or writes mcp.json; the LLM sees only servers explicitly opted in via `expose`, and the gate is re-checked under the manager lock at call time (no TOCTOU). A dedicated safety review before shipping found and fixed three issues, all pinned by tests in `tests/test_mcp_safety_review.py`: (F1) mcp.json missing from .gitignore - config could have been committed; (F2) the dashboard CSRF origin check used a prefix match so `http://localhost.evil.com` passed - now exact loopback hostname match on Origin AND Host header (blocks DNS-rebinding shapes); (F3) the expose-gate TOCTOU - flipping `mcp expose off` between the catalog check and the call let one invocation slip through - fixed with `call_tool(..., require_exposed=True)` re-reading config under the lock.

**Files**: agent_core/mcp/{__init__,jsonrpc,transports,config,client,manager}.py, agent_core/commands/mcp_cmd.py, agent.py (registry registration, `_nlp_mcp_tools`/`_nlp_mcp_call` handlers), agent_core/tool_schemas.py (+2 schemas), agent_core/monitoring/dashboard_api.py (do_POST + /mcp routes), static/mcp.html, .gitignore, USAGE.md (MCP section), AGENTS.md (architecture map). Tests: test_mcp_core.py (34, end-to-end over real pipes against tests/fixtures/fake_mcp_server.py incl. timeout-cap and kill-on-close proofs), test_mcp_cmd.py (15), test_mcp_dashboard.py (10, live HTTP round-trips incl. cross-origin rejection), test_mcp_nlp_bridge.py (11), test_mcp_safety_review.py (11). Full suite 1758 passed / 2 skipped; mypy clean on all new modules; audit_invariants OK.

## 2026-08-25 - fix: CI red since run #1 — undeclared optional deps (PIL, keyring, cryptography)

**Change**: pyproject.toml (new `vision` and `secrets` extras; `[dev]` now pulls pillow + cryptography + keyring so the full suite runs everywhere), tests/test_paste_image.py (`needs_pil` skipif marker on the 3 tests that build real PNGs — the other 10 tests in the file never touch PIL and now run on CI), tests/unit/test_secrets.py (`needs_backend` skipif on the 7 tests that call set_secret/resolve_secret, which raise RuntimeError when neither keyring nor cryptography is importable).

**Reason**: All 13 CI runs failed at "Run test suite" while the same tree passed locally. Root cause found by reproducing CI in a clean venv (`pip install -e .[dev]` only): tests/test_paste_image.py imports PIL at module scope (3× ModuleNotFoundError) and agent_core/security/secrets.py:107 raises "No secret backend available" without cryptography/keyring (7× RuntimeError) — both packages were used but never declared. The library itself degrades gracefully (lazy imports everywhere); only the tests assumed a dev machine.

**Files**: pyproject.toml, tests/test_paste_image.py, tests/unit/test_secrets.py. Verified in the clean venv before and after: 10 failed → 13 passed + 10 skipped with clear reasons; dev machine unchanged at 1664 passed / 2 skipped. Remaining undeclared imports (flask/marshmallow/redis/playwright in performance_dashboard + one test) are not imported by any collected test module and stay lazy — revisit if dashboard tests are ever wired into pytest.
