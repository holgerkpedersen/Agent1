...
against the current tree). Grouped by theme; ⚡ = quick win (hours),
🏗 = strategic. Status markers updated as items land.

## A. Wire up the dormant intelligence (highest leverage)

1. ⚡ **Inject decisions into chat_nlp** — STATUS: **DONE 2026-08-25**.
   ...
   the files read this session (`Agent._decision_constraints_block`).2. 🏗 **Real semantic memory** — `Agent._semantic_index` / `_knowledge_graph`
...
3. 🏗 **Make multillm consensus honest** — `ParallelRun.agree()` derives each
   ...
   plan).
4. 🏗 **Close the meta-policy loop** — `MetaPolicyEvolver.update_weights`,
...
5. ⚡ **Token-aware chat-history trimming** — `_trim_chat_history` keeps 60
...
10. 🏗 **Grow the repair catalog** — `harnessfix/repairs/` has exactly one
...
11. **LLM-tier diagnosis** — `diagnose.py` promises an LLM fallback tier when
    ...
    before building.
12. 🏗 **Agentic benchmark** — `benchmark.py` measures Q&A accuracy only;
...
13. 🏗 **Planner→executor split** — `src/agent1/orchestration/task_scheduler.py::DependencyGraph`
...
14. ⚡ **Plan-mode handoff** — plan-mode answers evaporate as terminal text;
    persist them to `.docs/<ts>/plan_proposed.md` and offer a one-command
    transition into build/implement.
15. ⚡ **multillm synthesis flag** — results print side-by-side then stop.
...
17. ⚡ **Dead duplicate modules** — `agent_core/tool_executor.py`,
